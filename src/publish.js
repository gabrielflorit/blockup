...
module.exports = function publish () {
  Promise.resolve()
    ...
    .then(
      title => new Promise((resolve, reject) => gistup(title, resolve, reject))
    )
    .catch(err => {
      if (err) {
        console.log()
...
function checkForGitRepo (resolve, reject) {
  ...
  if (fileExists('.git')) {
    console.log(
      chalk.yellow(
        'This is already a git repo. Update your block with standard git commands.'
      )
    )
    reject()
  } else {
...
function gistup (title, resolve, reject) {
  ...
  child.exec(`${gistupPath} ${gistupArgs}`, function (error, stdout, stderr) {
    ...
    if (gistHash) {
      ...
      console.log(
        chalk.bold(
          '\nBlock published! You can view it at',
          chalk.bold.blue(blockUrl)
        )
      )
    } else {
