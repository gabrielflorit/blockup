...
function checkFile (filenames, missingMessage) {
  ...
  console.log(
    chalk.red(
      `${indent}✖ ${filenames.join(' or ')}${missingMessage
        ? `. ${missingMessage}`
        : ''}`
    )
  )
...
module.exports = function checkMissingFiles () {
  const filesToCheck = [
    ...
    {
      filenames: ['preview.png', 'preview.jpg'],
      missingMessage: 'Expected a 960x500 image.'
    },
    { filenames: 'README.md' }
  ]

  console.log(chalk.dim('Checking for expected files:\n'))
  const isMissingFiles = filesToCheck.reduce(
    (anyMissing, file) =>
